...
#	if [ ! $filterOut -eq 1 ]; then
	  ARGS+=("$a")
#  fi
done

#
#if [ ${#SOURCES[@]} -eq 1 -a $showIncludesPresent -a -z "$sbrPath" ]; then
#  if [ ! -z "$objectPath" ]; then
#    sbrPath="${objectPath}.sbr"
#  else
#    sbrPath="${SOURCES[0]}.sbr"
#  fi
#	  ARGS+=("-Fr$sbrPath")
#fi
